...
test.beforeAll(async () => {
  ...
  _electronApp.process().stdout.on("data", (data) => console.log(`stdout: ${data}`));
  _electronApp.process().stderr.on("data", (error) => console.log(`stderr: ${error}`));
...
test("Microservices running", async () => {
  ...
  await firstWindow.waitForTimeout(
    (isWindows ? WINDOWS_TIMEOUT_SECONDS : DEFAULT_TIMEOUT_SECONDS) * MS_PER_SECOND,
  );
