  ...
  <v-container fluid class="pa-0 fill-height bg-transparent">
          ...
          >
            ...
            <template v-slot:item.geode_object="{ item }">
              <div class="d-flex flex-column py-2 align-start">
                <v-chip size="x-small" variant="outlined" class="type-chip mb-1 d-inline-flex">
                  {{ item.geode_object }}
                </v-chip>
                <span class="text-caption text-white-opacity-60">{{ getItemMetadata(item) }}</span>
              </div>
            </template>

            <template v-slot:item.created_at="{ item }">
              <span class="text-caption text-white-opacity-60">{{ formatSmartDate(item.created_at) }}</span>
            </template>
            ...
            <template v-slot:item.actions="{ item }">
              <div class="d-flex ga-1 justify-end">
                <v-btn icon size="small" variant="text" color="white" @click.stop="focusCamera(item)">
                  <v-icon>mdi-target</v-icon>
                  <v-tooltip activator="parent" location="top">Focus Camera</v-tooltip>
                </v-btn>
                <v-btn icon size="small" variant="text" color="white" @click.stop="isolateItem(item)">
                  <v-icon>mdi-filter-variant</v-icon>
                  <v-tooltip activator="parent" location="top">Isolate</v-tooltip>
                </v-btn>
                <v-btn icon size="small" variant="text" color="white" @click.stop="openRenameDialog(item)">
            ...
            <v-col v-for="item in filteredItems" :key="item.id" cols="12" sm="6" md="4" lg="3">
              <v-card variant="outlined" class="glass-card h-100">
                <v-card-text class="pa-4 text-white">
                  <div class="text-subtitle-1 font-weight-bold text-truncate mb-1">{{ item.displayed_name }}</div>
                  <v-chip size="x-small" color="primary" variant="flat" class="mb-4 d-inline-flex">{{ item.geode_object }}</v-chip>
                  <div class="text-caption text-white-opacity-60">{{ formatSmartDate(item.created_at) }}</div>
                </v-card-text>
                <v-card-actions class="bg-white-opacity-5">
                   <v-btn icon size="small" variant="text" color="white" @click.stop="toggleVisibility(item)">
    ...
    <v-dialog v-model="deleteSingleDialog" max-width="400" persistent>
      <v-card rounded="xl" class="bg-white text-center overflow-hidden">
        ...
        <v-avatar color="red-lighten-5" size="64" class="mt-8 mx-auto"><v-icon color="error" size="32">mdi-trash-can-outline</v-icon></v-avatar>
        <v-card-title class="text-h5 font-weight-bold text-grey-darken-4 pt-4">Delete Item</v-card-title>
        <v-card-text class="text-grey-darken-1 px-8">
          Are you sure you want to delete this item?
          <div class="bg-grey-lighten-4 rounded-lg pa-3 my-4 text-grey-darken-3 font-weight-bold border">{{ itemToDelete?.displayed_name }}</div>
        </v-card-text>
        <v-card-actions class="px-8 pb-8"><v-spacer /><v-btn variant="text" @click="deleteSingleDialog = false">Cancel</v-btn>
        <v-btn color="error" variant="flat" class="px-8 rounded-pill" @click="executeDelete">Delete</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteSelectedDialog" max-width="400" persistent>
      <v-card rounded="xl" class="bg-white text-center overflow-hidden">
        <div class="bg-error w-100" style="height: 6px"></div>
        <v-avatar color="red-lighten-5" size="64" class="mt-8 mx-auto"><v-icon color="error" size="32">mdi-alert-circle-outline</v-icon></v-avatar>
        <v-card-title class="text-h5 font-weight-bold text-grey-darken-4 pt-4">Delete Items</v-card-title>
        <v-card-text class="text-grey-darken-1 px-8">
          Are you sure you want to delete <strong>{{ selected.length }}</strong> items?
        </v-card-text>
        <v-card-actions class="px-8 pb-8"><v-spacer /><v-btn variant="text" @click="deleteSelectedDialog = false">Cancel</v-btn>
        <v-btn color="error" variant="flat" class="px-8 rounded-pill" @click="deleteSelected">Delete All</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="renameDialog" max-width="400">
      <v-card rounded="xl" class="bg-white text-center overflow-hidden">
        ...
        <v-avatar color="blue-lighten-5" size="64" class="mt-8 mx-auto"><v-icon color="primary" size="32">mdi-pencil-outline</v-icon></v-avatar>
        <v-card-title class="text-h5 font-weight-bold text-grey-darken-4 pt-4">Rename Item</v-card-title>
        <v-card-text class="px-8">
          <v-text-field v-model="newItemName" label="New Name" variant="outlined" color="primary" class="mt-4" @keydown.enter="confirmRename" />
        </v-card-text>
        <v-card-actions class="px-8 pb-8"><v-spacer /><v-btn variant="text" @click="renameDialog = false">Cancel</v-btn>
        <v-btn color="primary" variant="flat" class="px-8 rounded-pill" @click="confirmRename">Rename</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(i => i.displayed_name?.toLowerCase().includes(lower))
})

async function loadData() {
  loading.value = true
  try { items.value = await dataBaseStore.getAllItems() } 
...
async function toggleVisibility(item) {
  ...
  if (newVisible) {
    await treeviewStore.addItem(item.geode_object, item.displayed_name, item.id, item.viewer_type)
  } else {
...
async function confirmRename() {
  ...
  try {
    ...
    await dataBaseStore.updateItem(itemToRename.value.id, { displayed_name: newItemName.value })
    treeviewStore.removeItem(itemToRename.value.id)
    await treeviewStore.addItem(itemToRename.value.geode_object, newItemName.value, itemToRename.value.id, itemToRename.value.viewer_type)
    renameDialog.value = false
    showFeedback("Renamed successfully")
  } catch (e) { showFeedback("Failed to rename", "error") }
}
