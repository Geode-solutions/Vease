  ...
  <v-container fluid class="pa-0 fill-height bg-transparent">
    <v-row no-gutters class="fill-height">
      <v-col class="d-flex flex-column fill-height overflow-hidden">
        <DataManagerHeader v-model:search-value="search" v-model:active-tab="activeTab" :tabs="UIStore.dataManagerTabs"
          ref="headerRef" />
        ...
        <v-window v-model="activeTab" class="flex-grow-1 overflow-hidden pa-4">
          <v-window-item value="data" class="fill-height overflow-y-auto">
            <BatchActionBanner :selected-count="selectedIds.length" @delete="deleteSelectedDialog = true"
              @clear="selectedIds = []" />

            <DataTable v-if="viewMode === 'list'" v-model:selected-ids="selectedIds" :items="items" :search="search"
              @toggle-visibility="toggleVisibility" @focus-camera="focusCamera" @isolate="isolateItem"
              @rename="openRenameDialog" @delete="confirmDelete" />
          </v-window-item>

          <v-window-item v-for="tab in UIStore.datamanagerTabs" :key="tab.id" :value="tab.id"
    ...
    <DeleteDialog v-model:show="deleteSelectedDialog" :selected-count="selectedIds.length" @confirm="deleteSelected" />

    <RenameDialog v-model:show="renameDialog" :item="itemToRename" :initial-name="newItemName"
