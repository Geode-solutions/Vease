<template>
  <v-data-table :model-value="selectedIds" @update:model-value="$emit('update:selectedIds', $event)" :headers="headers"
    :items="items" :search="search" show-select fixed-header :height="selectedIds.length > 0 ? '480' : '550'"
    item-value="id" return-object class="bg-transparent border-thin rounded-lg text-white custom-scrollbar">
