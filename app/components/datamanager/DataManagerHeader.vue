<template>
  <v-sheet class="pa-6 pb-2" rounded="xl" color="transparent">
    <v-tabs :model-value="activeTab" @update:model-value="$emit('update:activeTab', $event)" bg-color="transparent"
      color="primary" class="mb-6 border-b-thin" height="48" selected-class="bg-primary text-white">
      <v-tab value="data" class="text-none font-weight-medium px-6 rounded-t-lg"
        :variant="activeTab === 'data' ? 'elevated' : 'text'" :elevation="activeTab === 'data' ? 4 : 0"
        :border="activeTab === 'data' ? true : false">
        <v-icon start size="20">mdi-database</v-icon>
        Data Manager
      </v-tab>

      <v-tab v-for="tab in tabs" :key="tab.id" :value="tab.id" class="text-none font-weight-medium px-6 rounded-t-lg"
        :variant="activeTab === tab.id ? 'elevated' : 'text'" :elevation="activeTab === tab.id ? 4 : 0">
