<template>
  <v-card :width="500" elevation="0" class="pa-4">
    <v-card-title class="pb-2 text-h6 text-primary font-weight-bold d-flex align-center">
      <v-img src="@vease/assets/img/voi.svg" alt="AOI icon" class="mr-3" width="48" height="48" />
    ...
    <v-card-text>
      <v-form ref="form">
        <v-text-field label="VOI Name" v-model="name" prepend-inner-icon="mdi-format-title" type="text"
          variant="outlined" color="primary" :rules="[(v) => !!v || 'Name is required']" required class="mb-4" />

        <v-select label="Select AOI" v-model="selectedAOI" :items="aoiList" item-title="name" item-value="id"
          prepend-inner-icon="mdi-vector-square" variant="outlined" color="primary"
          :rules="[(v) => !!v || 'AOI selection is required']" required class="mb-4"
          no-data-text="No AOI available. Please create an AOI first." @update:modelValue="onAOISelected">
        ...
        <div class="mb-4">
          ...
          <v-row dense>
            <v-col cols="6">
              <v-text-field label="Z Min" v-model="z_min" prepend-inner-icon="mdi-axis-z-arrow" type="text"
                inputmode="decimal" variant="outlined" color="secondary" density="comfortable" :rules="[
            ...
            <v-col cols="6">
              <v-text-field label="Z Max" v-model="z_max" prepend-inner-icon="mdi-axis-z-arrow" type="text"
                inputmode="decimal" variant="outlined" color="secondary" density="comfortable" :rules="[
    ...
    <v-card-actions class="px-4 pb-4">
      <v-btn variant="text" color="grey-darken-1" size="large" @click="openCreateTools" :disabled="loading"
        class="text-none">
        <v-icon start>mdi-close-circle-outline</v-icon>
        Cancel
      </v-btn>

      <v-btn color="primary" size="large" variant="flat" :loading="loading" :disabled="!isFormFilled" @click="createVOI"
